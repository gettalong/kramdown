...
module Kramdown::Converter::MathEngine
  ...
  # Consider this a lightweight alternative to MathjaxNode.
  # Uses KaTeX and ExecJS instead of MathJax and Node.js.
  ...
  module SsKaTeX
    ...
    private_constant :ENV_EXECJS_RUNTIME
    ...
    # specific _jsrun_ because those depend on configuration not given here.
    # This test mainly indicates whether static dependencies such as the +execjs+ gem are available.
    AVAILABLE = begin
      require 'json'
      # Defer automatic JS engine selection
      ENV['EXECJS_RUNTIME'] = 'Disabled'
      require 'execjs'
      true
    rescue LoadError  # ExecJS missing
      false
    ensure
    ...
    if AVAILABLE
      ...
      private_constant :DATADIR

      # The default for the +:jsdir+ option in +math_engine_opts+.
      # Path of a directory with Javascript helper files.
      DEFAULT_JSDIR = File.join(DATADIR, 'js')
      private_constant :DEFAULT_JSDIR

      # The default path to +katex.js+, cf. the +:katexjs+ option in +math_engine_opts+.
      # For a relative path, the starting point is the current working directory.
      DEFAULT_KATEXJS = File.join('katex', 'katex.min.js')
      private_constant :DEFAULT_KATEXJS

      # The default for the +:libs+ option in +math_engine_opts+.
      # A list of UTF-8-encoded Javascript helper files to load.
      # Relative paths are interpreted relative to _jsdir_.
      DEFAULT_LIBS = ['escape_nonascii_html.js', 'tex_to_html.js']
      private_constant :DEFAULT_LIBS

      # A positive integer limiting the number of cached Javascript engine contexts.
      JSCTX_MAX = 10
      private_constant :JSCTX_MAX

      # Class-level cache for JS engine context, queried by configuration.
      # Note: JSCTX contents may become stale if the contents of used JS files change while the
      # configuration remains unchanged.
      JSCTX = ::Kramdown::Utils::LRUCache.new(JSCTX_MAX)
      private_constant :JSCTX
      ...
      private_constant :JSRUN_FROMSYM
      # Separately assigning and filling JSRUN_FROMSYM would be valid Ruby,
      # but then Rdoc would hallucinate some definition in +SsKaTeX::ExecJS+.

      # Subclasses of +ExecJS::Runtime+ provide +.name+ (too verbose), but not, say, +.symbol+.
      # This dictionary associates each JS runtime class with a representative symbol.
      # For aliases like <tt>SpiderMonkey = Spidermonkey</tt>, an unspecified choice is made.
      JSRUN_TOSYM = JSRUN_FROMSYM.invert
      private_constant :JSRUN_TOSYM
      ...
      private
        ...
        # Configuration dicts contain keys in both string and symbol form.
        ...
        def dedup_keys(conf)
          ...
          case conf
          when Hash
            conf.reject do |key, value|
              key.is_a?(Symbol) && conf.has_key?(key.to_s)
            # end.transform_values! { |value| dedup_keys(value) }  # Ruby >= 2.4
            end.tap do |dict|
              dict.each do |key, value|
                dict[key] = dedup_keys(value)
              end
            end
          when Array
        ...
        # Return a closure that logs verbose-level messages to the appropriate channel(s).
        # With the debug option set, the debug channel gets a copy as well.
        # Usage:
        ...
        # with the converter's +math_engine_opts+.
        # Cache the engine context reference for reuse.
        def js_context(converter)
          config = converter.options[:math_engine_opts]
          JSCTX[config] ||= begin
            log = logger(converter)
            # Configure JS engine. If specified, config[:jsrun] takes precedence, followed by
            # an equivalent of ExecJS::Runtimes.autodetect, i.e. ENV['EXECJS_RUNTIME']
            # (parsed in a less adventurous way) or else the best available engine.
            # If the result is invalid, unknown, or unavailable, errors will ensue.
            jsrun = (config[:jsrun] ||
                     ENV_EXECJS_RUNTIME ||
                     JSRUN_TOSYM[ExecJS::Runtimes.best_available] ||
